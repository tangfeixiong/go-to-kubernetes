...
func checkIfMatch(w http.ResponseWriter, r *http.Request) condResult {
	...
	for {
		...
		if etagStrongMatch(etag /*, w.Header().get("Etag")*/, header_get(w.Header(), "Etag")) {
			return condTrue
		}
		im = remain
	}
...
func checkIfNoneMatch(w http.ResponseWriter, r *http.Request) condResult {
	inm := /*r.Header.get("If-None-Match")*/ header_get(r.Header, "If-None-Match")
	if inm == "" {
		return condNone
	}
	buf := inm
	for {
		...
		if etagWeakMatch(etag /*, w.Header().get("Etag")*/, header_get(w.Header(), "Etag")) {
			return condFalse
		}
		buf = remain
	}
...
func checkIfRange(w http.ResponseWriter, r *http.Request, modtime time.Time) condResult {
	...
	ir := /*r.Header.get("If-Range")*/ header_get(r.Header, "If-Range")
	if ir == "" {
		return condNone
	}
	etag, _ := scanETag(ir)
...
func checkPreconditions(w http.ResponseWriter, r *http.Request, modtime time.Time) (done bool, rangeHeader string) {
	...
	rangeHeader = /*r.Header.get("Range")*/ header_get(r.Header, "Range")
	if rangeHeader != "" {
